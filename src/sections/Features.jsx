                            ...
                            <div
                                ...
                                <div className="w-full flex items-start justify-start">
                                    <div className="-ml-3 mb-12 flex items-center justify-center flex-col">
                                        ...
                                        <img src={icon} className="size-28" alt="icon"/>
                                    </div>
                                </div>

                                <p className="caption mb-5 max-md:mb-6">{caption}</p>
                                <h2 className="max-w-400 mb-7 h3 text-p4 max-md:mb-6 max-md:h5">{title}</h2>
                                <p className="mb-11 body-1 max-md:mb-8 max-md:body">{text}</p>
                                <Button icon={button.icon}>{button.title}</Button>
                            </div>
                        ))}
                        <ul className="relative flex justify-around flex-grow px-[5%] border-2 border-s3 rounded-7xl max-md:hidden">
                            <div className="absolute bg-s3/20 top-[38%] left-0 right-0 w-full h-[1pc] z-10"></div>
